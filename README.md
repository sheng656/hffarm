...
点击顶部「📊 Excel大表」切换按钮或导航菜单直达全量明细大表，像在 Excel 中一样平铺审阅当日所有细节。

![收菜全量明细大表](docs/screenshots/detail_page.png)

**核心特性：**
- **默认平铺全量表 (Flat Grid)**：一目了然显示全部细项，支持按团队/按产品分组层级切换
- **表头交互升降序**：点击团队、产品名、大棚、箱型、总筐数或时间列，轻松进行升降序重排
- **多维组合筛选**：搜索框模糊检索 + 团队/箱型按需过滤
- **底栏 Grand Total**：实效展现全量条数、包数、散数与总筐数合计
