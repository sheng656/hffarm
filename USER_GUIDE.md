...
**位置**：首页/历史页顶部点击「📊 Excel大表」切换，或底部「更多」→「收菜明细大表」

![收菜全量明细大表](docs/screenshots/detail_page.png)

像使用 Excel 电子表格一样体验全量收运明细：

- **平铺全量展现**：默认以平铺大表展示全部收菜记录，一目了然
- **表头快速排序**：点击团队、产品名称、大棚、箱型、总筐数、录入时间等表头，快速升降序重排
- **视图分组切换**：可随时自由切换为「按团队分组」、「按产品分组」或「平铺全量表」
- **多维筛选过滤**：支持搜索框关键字过滤，可搭配团队按钮和箱型下拉框组合筛选
- **底栏 Grand Total**：清晰显示汇总计算的记录数、包数、散数和总筐数
